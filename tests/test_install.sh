...
WIDGET_DIR=".local/share/claudebar/quickshell"
OLD_WIDGET_DIR=".config/quickshell/claudebar"
...
check_contains "autostart runs the widget daemon" "$(cat "$H/.config/autostart/claudebar.desktop")" "Exec=$H/bin/qs -p $H/$WIDGET_DIR -d"
...
check_contains "starts the widget" "$(cat "$H/qs.log")" "-p $H/$WIDGET_DIR -d"
check_false "puts nothing in ~/.config/quickshell" test -e "$H/.config/quickshell"
...
ln -s "$ROOT/quickshell" "$H/$OLD_WIDGET_DIR"
ln -s "$ROOT/gnome-extension" "$H/$EXTENSION_DIR"
install "$H"
check "exits 0" "$RC" "0"
check_false "removes the old widget link" test -e "$H/$OLD_WIDGET_DIR" -o -L "$H/$OLD_WIDGET_DIR"
check_true "copies the widget to the new place" test -f "$H/$WIDGET_DIR/shell.qml"
...
check_contains "stops the widget started from the old place" "$(cat "$H/qs.log")" "-p $H/$OLD_WIDGET_DIR kill"
rm -rf "$H"

section "an update moves a copy out of ~/.config/quickshell"
H="$(setup_home)"
before="$(snapshot "$H")"
mkdir -p "$H/.config/quickshell"
cp -r "$ROOT/quickshell" "$H/$OLD_WIDGET_DIR"
mkdir -p "$H/.local/state/claudebar" && printf 'dir %s\nfiles\n' "$H/.config/quickshell" >"$H/.local/state/claudebar/installed"
install "$H"
check "exits 0" "$RC" "0"
check_false "removes the old copy" test -e "$H/$OLD_WIDGET_DIR"
check_true "copies the widget to the new place" test -f "$H/$WIDGET_DIR/shell.qml"
run_script "$H" "$H/$UNINSTALLER"
check "the uninstall still leaves HOME as it was" "$(snapshot "$H")" "$before"
rm -rf "$H"

section "a claudebar Quickshell config of the user's own is kept"
H="$(setup_home)"
mkdir -p "$H/$OLD_WIDGET_DIR" && echo 'ShellRoot {}' >"$H/$OLD_WIDGET_DIR/shell.qml"
install "$H"
check "exits 0" "$RC" "0"
run_script "$H" "$H/$UNINSTALLER"
check "keeps it through install and uninstall" "$(cat "$H/$OLD_WIDGET_DIR/shell.qml")" "ShellRoot {}"
rm -rf "$H"
...
check_contains "stops the widget" "$(cat "$H/qs.log")" "-p $H/$WIDGET_DIR kill"
rm -rf "$H"
